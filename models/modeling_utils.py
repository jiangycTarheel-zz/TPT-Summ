...
class PreTrainedModel(nn.Module, ModuleUtilsMixin):
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_name_or_path, return_unpretrained=False, *model_args, **kwargs):
    ...
    def _generate_no_beam_search(
        ...
        self.dec_self_role_attn, self.dec_cross_role_attn = [], []
        ...
        while cur_len < max_length:
            ...
            if role_attn_vis:
                #self.dec_self_role_attn.append(self.decoder.block[-1].layer[0].SelfAttention.role_weights)
                #self.dec_cross_role_attn.append(self.decoder.block[-1].layer[1].EncDecAttention.role_weights)
                self.dec_self_role_attn.append(self.decoder.block[-1].layer[1].role_weights.unsqueeze(1))
                self.dec_cross_role_attn.append(self.decoder.block[-1].layer[1].role_weights.unsqueeze(1))
        ...
        self.token_probs = torch.cat(self.token_probs)

        if role_attn_vis:
            self.dec_cross_role_attn = torch.cat(self.dec_cross_role_attn, dim=2)
            self.dec_self_role_attn = torch.cat(self.dec_self_role_attn, dim=2)
